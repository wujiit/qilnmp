...
Upgrade_Nginx() {
  ...
  while :; do echo
    ...
    if [ "${NEW_nginx_ver}" != "${OLD_nginx_ver}" ]; then
      ...
      if [ -e "nginx-${NEW_nginx_ver}.tar.gz" ]; then
        src_url=${mirror_link}/src/openssl-${openssl3_ver}.tar.gz && Download_src
        src_url=${mirror_link}/src/pcre-${pcre_ver}.tar.gz && Download_src
        src_url=${mirror_link}/src/ngx_devel_kit.tar.gz && Download_src
        src_url=${mirror_link}/src/lua-nginx-module-${lua_nginx_module_ver}.tar.gz && Download_src
  ...
  if [ -e "nginx-${NEW_nginx_ver}.tar.gz" ]; then
    ...
    rm -rf $$
    nginx_configure_args=`echo ${nginx_configure_args_tmp} | sed "s@lua-nginx-module-\w.\w\+.\w\+ @lua-nginx-module-${lua_nginx_module_ver} @" | sed "s@lua-nginx-module @lua-nginx-module-${lua_nginx_module_ver} @" | sed "s@--with-openssl=../openssl-[^ ]\+ @--with-openssl=../openssl-${openssl3_ver} @" | sed "s@--with-pcre=../pcre-\w.\w\+ @--with-pcre=../pcre-${pcre_ver} @"`
    if [ -n "`echo $nginx_configure_args | grep lua-nginx-module`" ]; then
      ${oneinstack_dir}/upgrade.sh --oneinstack > /dev/null
      src_url=${mirror_link}/src/luajit2-${luajit2_ver}.tar.gz && Download_src
      ...
      src_url=${mirror_link}/src/lua-resty-core-${lua_resty_core_ver}.tar.gz && Download_src
      ...
      src_url=${mirror_link}/src/lua-resty-lrucache-${lua_resty_lrucache_ver}.tar.gz && Download_src
...
Upgrade_Tengine() {
  ...
  if [ -e "tengine-${NEW_tengine_ver}.tar.gz" ]; then
    ...
    rm -rf $$
    tengine_configure_args=`echo ${tengine_configure_args_tmp} | sed "s@--with-openssl=../openssl-[^ ]\+ @--with-openssl=../openssl-${openssl3_ver} @" | sed "s@--with-pcre=../pcre-\w.\w\+ @--with-pcre=../pcre-${pcre_ver} @"`
    export LUAJIT_LIB=/usr/local/lib
    export LUAJIT_INC=/usr/local/include/luajit-2.1
