...
Install_PHP84() {
  ...
  # Check if system libxml2 version is sufficient (PHP 8.4+ requires >= 2.9.4)
  ...
  if [ ! -e "${curl_install_dir}/lib/libcurl.la" ]; then
    ...
    ./configure --prefix=${curl_install_dir} ${php84_with_ssl} ${with_nghttp2} --without-libpsl || {
  ...
  Ensure_source_archive "php-${php84_ver}.tar.gz" "https://www.php.net/distributions/php-${php84_ver}.tar.gz"
  rm -rf php-${php84_ver}
  tar xzf php-${php84_ver}.tar.gz || {
    echo "${CFAILURE}Extract php-${php84_ver}.tar.gz failed, source package may be corrupted.${CEND}"
    kill -9 $$; exit 1;
  }
  [ ! -d "php-${php84_ver}" ] && {
    echo "${CFAILURE}PHP source directory php-${php84_ver} not found after extract.${CEND}"
    kill -9 $$; exit 1;
  }
