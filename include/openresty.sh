...
Ensure_perl_ipc_cmd_for_web() {
  ...
  perl -MTime::Piece -e1 >/dev/null 2>&1 || missing_modules="${missing_modules} Time::Piece"
  [ -z "${missing_modules}" ] || {
    echo "${CFAILURE}Required Perl modules for OpenSSL/OpenResty build are still missing:${missing_modules}. Install them manually and retry.${CEND}"
    kill -9 $$; exit 1;
  }
}
...
Install_OpenResty() {
  ...
  openresty_build_thread=${NGINX_THREAD:-${THREAD}}
  [ -z "${openresty_build_thread}" ] && openresty_build_thread=1
  make -j ${openresty_build_thread} || {
    if [ ${openresty_build_thread} -gt 1 ]; then
      echo "${CWARNING}OpenResty compile failed with -j${openresty_build_thread}, retrying with -j1...${CEND}"
      make -j 1 || { echo "${CFAILURE}OpenResty compile failed with -j1.${CEND}"; kill -9 $$; exit 1; }
    else
      echo "${CFAILURE}OpenResty compile failed.${CEND}"
      kill -9 $$; exit 1;
    fi
  }
  make install
  if [ -e "${openresty_install_dir}/nginx/conf/nginx.conf" ]; then
    popd > /dev/null
    rm -rf pcre-${pcre_ver}* openssl-${openssl3_ver}* openresty-${openresty_ver}*
    echo "${CSUCCESS}OpenResty installed successfully! ${CEND}"
  else
    rm -rf pcre-${pcre_ver}* openssl-${openssl3_ver}* openresty-${openresty_ver}* ${openresty_install_dir}*
  ...
  if [ "${apache_flag}" == 'y' ] || [ -e "${apache_install_dir}/bin/httpd" ]; then
    ...
    [[ "${php_option}" =~ ^[1-9]$|^1[0-5]$ ]] && [ -z "`grep '/php-fpm_status' ${openresty_install_dir}/nginx/conf/nginx.conf`" ] &&  sed -i "s@index index.html index.php;@index index.html index.php;\n    location ~ /php-fpm_status {\n        #fastcgi_pass remote_php_ip:9000;\n        fastcgi_pass unix:/dev/shm/php-cgi.sock;\n        fastcgi_index index.php;\n        include fastcgi.conf;\n        allow 127.0.0.1;\n        deny all;\n        }@" ${openresty_install_dir}/nginx/conf/nginx.conf
  fi
