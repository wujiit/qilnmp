...
Install_MySQL82() {
  ...
  if [ "${dbinstallmethod}" == "1" ]; then
    if [ -f "mysql-${mysql82_ver}-linux-glibc2.17-x86_64.tar.xz" ]; then
      tar xJf mysql-${mysql82_ver}-linux-glibc2.17-x86_64.tar.xz
      mv mysql-${mysql82_ver}-linux-glibc2.17-x86_64/* ${mysql_install_dir}
    elif [ -f "mysql-${mysql82_ver}-linux-glibc2.28-x86_64.tar.xz" ]; then
      tar xJf mysql-${mysql82_ver}-linux-glibc2.28-x86_64.tar.xz
      mv mysql-${mysql82_ver}-linux-glibc2.28-x86_64/* ${mysql_install_dir}
    else
      echo "${CFAILURE}MySQL 8.2 binary package not found, expected glibc2.17 or glibc2.28 tarball.${CEND}"
      kill -9 $$; exit 1;
    fi
