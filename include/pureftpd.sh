...
Install_PureFTPd() {
  ...
  if [ -e "${pureftpd_install_dir}/sbin/pure-ftpwho" ]; then
    ...
    openssl req -x509 -days 7300 -sha256 -nodes -subj "/C=CN/ST=Shanghai/L=Shanghai/O=QILNMP/CN=${IPADDR}" -newkey rsa:2048 -keyout /etc/ssl/private/pure-ftpd.pem -out /etc/ssl/private/pure-ftpd.pem
