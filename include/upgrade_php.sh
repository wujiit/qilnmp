...
Upgrade_PHP() {
  ...
  while :; do echo
    ...
    if [ "${NEW_php_ver%.*}" == "${OLD_php_ver%.*}" ]; then
      if [ ! -e "php-${NEW_php_ver}.tar.gz" ]; then
        src_url=${mirror_link}/src/php-${NEW_php_ver}.tar.gz
        src_url_backup=https://www.php.net/distributions/php-${NEW_php_ver}.tar.gz
  ...
  if [ -e "php-${NEW_php_ver}.tar.gz" ]; then
    ...
    [ -d "/usr/local/zlib/lib/pkgconfig" ] && export PKG_CONFIG_PATH=/usr/local/zlib/lib/pkgconfig:$PKG_CONFIG_PATH
