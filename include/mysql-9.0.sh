...
Install_MySQL90() {
  ...
  if [ "${dbinstallmethod}" == "1" ]; then
    if [ -f "mysql-${mysql90_ver}-linux-glibc2.28-x86_64.tar.xz" ]; then
      tar xJf mysql-${mysql90_ver}-linux-glibc2.28-x86_64.tar.xz
      mv mysql-${mysql90_ver}-linux-glibc2.28-x86_64/* ${mysql_install_dir}
    else
      echo "${CFAILURE}MySQL 9.0 binary package not found, expected mysql-${mysql90_ver}-linux-glibc2.28-x86_64.tar.xz.${CEND}"
      kill -9 $$; exit 1;
    fi
    ...
    tar xzf mysql-${mysql90_ver}.tar.gz
    pushd mysql-${mysql90_ver}
    mysql_build_thread=${DB_THREAD:-${THREAD}}
    [ -z "${mysql_build_thread}" ] && mysql_build_thread=1
    ...
    make -j ${mysql_build_thread} || {
      if [ ${mysql_build_thread} -gt 1 ]; then
        echo "${CWARNING}MySQL compile failed with -j${mysql_build_thread}, retrying with -j1...${CEND}"
        make -j 1 || { echo "${CFAILURE}MySQL compile failed with -j1.${CEND}"; kill -9 $$; exit 1; }
      else
        echo "${CFAILURE}MySQL compile failed.${CEND}"
        kill -9 $$; exit 1;
      fi
    }
    make install
  ...
  if [ -d "${mysql_install_dir}/support-files" ]; then
    ...
    if [ "${dbinstallmethod}" == "1" ]; then
      rm -rf mysql-${mysql90_ver}-*-x86_64
    elif [ "${dbinstallmethod}" == "2" ]; then
      rm -rf mysql-${mysql90_ver} boost_${boostVersion2}
    fi
  else
