  ...
  if [ -e "${php_install_dir}/bin/phpize" ]; then
    ...
    if [[ "${PHP_main_ver}" =~ ^7.[0-4]$|^8.[0-5]$ ]]; then
      if [[ "${PHP_main_ver}" =~ ^7.[0-1]$ ]]; then
        src_url=${mirror_link}/src/xdebug-${xdebug_oldver}.tgz
        src_url_backup=https://pecl.php.net/get/xdebug-${xdebug_oldver}.tgz
        Download_src; unset src_url_backup
        tar xzf xdebug-${xdebug_oldver}.tgz
        pushd xdebug-${xdebug_oldver} > /dev/null
      else
        src_url=${mirror_link}/src/xdebug-${xdebug_ver}.tgz
        src_url_backup=https://pecl.php.net/get/xdebug-${xdebug_ver}.tgz
        Download_src; unset src_url_backup
