...
Install_MySQL80() {
  ...
  if [ -d "${mysql_install_dir}/support-files" ]; then
    ...
    if [ "${dbinstallmethod}" == "1" ]; then
      rm -rf mysql-${mysql80_ver}-linux-glibc2.28-x86_64 mysql-${mysql80_ver}-linux-glibc2.12-x86_64
    elif [ "${dbinstallmethod}" == "2" ]; then
