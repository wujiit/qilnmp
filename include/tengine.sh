...
Ensure_perl_ipc_cmd_for_web() {
  ...
  perl -MTime::Piece -e1 >/dev/null 2>&1 || missing_modules="${missing_modules} Time::Piece"
  [ -z "${missing_modules}" ] || {
    echo "${CFAILURE}Required Perl modules for OpenSSL/Tengine build are still missing:${missing_modules}. Install them manually and retry.${CEND}"
    kill -9 $$; exit 1;
  }
}
...
Install_Tengine() {
  ...
  ./configure --prefix=${tengine_install_dir} --user=${run_user} --group=${run_group} --with-http_v2_module --with-http_ssl_module --with-stream --with-stream_ssl_preread_module --with-stream_ssl_module --with-http_gzip_static_module --with-http_realip_module --with-http_flv_module --with-http_mp4_module --with-openssl=../openssl-${openssl3_ver} --with-pcre=../pcre-${pcre_ver} --with-pcre-jit --with-jemalloc ${nginx_modules_options}
  tengine_build_thread=${NGINX_THREAD:-${THREAD}}
  [ -z "${tengine_build_thread}" ] && tengine_build_thread=1
  make -j ${tengine_build_thread} || {
    if [ ${tengine_build_thread} -gt 1 ]; then
      echo "${CWARNING}Tengine compile failed with -j${tengine_build_thread}, retrying with -j1...${CEND}"
      make -j 1 || { echo "${CFAILURE}Tengine compile failed with -j1.${CEND}"; kill -9 $$; exit 1; }
    else
      echo "${CFAILURE}Tengine compile failed.${CEND}"
      kill -9 $$; exit 1;
    fi
  }
  make install
  if [ -e "${tengine_install_dir}/conf/nginx.conf" ]; then
    popd > /dev/null
    rm -rf pcre-${pcre_ver}* openssl-${openssl3_ver}* tengine-${tengine_ver}*
    echo "${CSUCCESS}Tengine installed successfully! ${CEND}"
  else
    rm -rf pcre-${pcre_ver}* openssl-${openssl3_ver}* tengine-${tengine_ver}* rm -rf ${tengine_install_dir}*
