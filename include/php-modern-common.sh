...
PHP_Modern_ensure_source_archive() {
  local file_name=$1
  local backup_url=$2

  [ -z "${file_name}" ] && return 1
  if [ -s "${file_name}" ] && PHP_Modern_is_valid_archive "${file_name}"; then
    return 0
  fi
...
Install_Modern_PHP() {
  local php_version=$1
  local php_with_ssl=$2
  local php_with_curl=$3
  local php_with_openssl=$4
  local php_build_thread=""
  local php_ext_dir=""
  local sys_libxml2_ver="0"
  local sys_zlib_ver="0"
  local argon2_src_dir=""
  local with_nghttp2=""
  local with_zlib=""

  [ -z "${php_version}" ] && {
    echo "${CFAILURE}PHP version is required for Install_Modern_PHP.${CEND}"
    kill -9 $$; exit 1;
  }
  ...
  if PHP_Modern_version_lt "${sys_libxml2_ver}" "2.9.4"; then
    PHP_Modern_ensure_source_archive "libxml2-${libxml2_ver}.tar.xz" "https://download.gnome.org/sources/libxml2/$(echo ${libxml2_ver} | awk -F. '{print $1\".\"$2}')/libxml2-${libxml2_ver}.tar.xz"
  ...
  if [ ! -e "/usr/local/lib/libiconv.la" ]; then
    PHP_Modern_ensure_source_archive "libiconv-${libiconv_ver}.tar.gz" "https://ftp.gnu.org/pub/gnu/libiconv/libiconv-${libiconv_ver}.tar.gz"
  ...
  if [ ! -e "${curl_install_dir}/lib/libcurl.la" ]; then
    PHP_Modern_ensure_source_archive "curl-${curl_ver}.tar.gz" "https://curl.se/download/curl-${curl_ver}.tar.gz"
    tar xzf curl-${curl_ver}.tar.gz
    pushd curl-${curl_ver} > /dev/null
    [ -e "/usr/local/lib/libnghttp2.so" ] && with_nghttp2='--with-nghttp2=/usr/local'
    ./configure --prefix=${curl_install_dir} ${php_with_ssl} ${with_nghttp2} --without-libpsl || {
  ...
  if [ ! -e "${freetype_install_dir}/lib/libfreetype.la" ]; then
    PHP_Modern_ensure_source_archive "freetype-${freetype_ver}.tar.gz" "https://downloads.sourceforge.net/project/freetype/freetype2/${freetype_ver}/freetype-${freetype_ver}.tar.gz"
  ...
  if [ ! -e "/usr/local/lib/pkgconfig/libargon2.pc" ]; then
    PHP_Modern_ensure_source_archive "argon2-${argon2_ver}.tar.gz" "https://github.com/P-H-C/phc-winner-argon2/archive/refs/tags/${argon2_ver}.tar.gz"
  ...
  if [ ! -e "/usr/local/lib/libsodium.la" ]; then
    PHP_Modern_ensure_source_archive "libsodium-${libsodium_up_ver}.tar.gz" "https://download.libsodium.org/libsodium/releases/libsodium-${libsodium_up_ver}.tar.gz"
  ...
  if [ ! -e "/usr/local/lib/libzip.so" -a ! -e "/usr/local/lib/libzip.la" ]; then
    PHP_Modern_ensure_source_archive "libzip-${libzip_ver}.tar.gz" "https://libzip.org/download/libzip-${libzip_ver}.tar.gz"
  ...
  if [ ! -e "/usr/local/include/mhash.h" -a ! -e "/usr/include/mhash.h" ]; then
    PHP_Modern_ensure_source_archive "mhash-${mhash_ver}.tar.gz" "https://downloads.sourceforge.net/project/mhash/mhash/${mhash_ver}/mhash-${mhash_ver}.tar.gz"
  ...
  if PHP_Modern_version_lt "${sys_zlib_ver}" "1.2.11"; then
    PHP_Modern_ensure_source_archive "zlib-${zlib_ver}.tar.gz" "https://zlib.net/zlib-${zlib_ver}.tar.gz"
  ...
  PHP_Modern_ensure_source_archive "php-${php_version}.tar.gz" "https://www.php.net/distributions/php-${php_version}.tar.gz"
  rm -rf php-${php_version}
  tar xzf php-${php_version}.tar.gz || {
    echo "${CFAILURE}Extract php-${php_version}.tar.gz failed, source package may be corrupted.${CEND}"
    kill -9 $$; exit 1;
  }
  [ ! -d "php-${php_version}" ] && {
    echo "${CFAILURE}PHP source directory php-${php_version} not found after extract.${CEND}"
    kill -9 $$; exit 1;
  }
  ...
  make ZEND_EXTRA_LIBS='-liconv' -j ${php_build_thread} || {
    if [ ${php_build_thread} -gt 1 ]; then
      ...
      make clean
      make ZEND_EXTRA_LIBS='-liconv' -j 1 || {
        echo "${CFAILURE}PHP compile failed with -j1.${CEND}"
        kill -9 $$; exit 1;
      }
    else
